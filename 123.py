...
for x in range(epoch):
    # if(1):
    #     resnet18.load_state_dict(torch.load('./data/model.pth'))
    arange = np.arange(0, (x+1)*250)
    print("x的数字：{}".format(x))
    dx_train = torch.utils.data.dataset.Subset(ds_train, arange)
    dl_train = torch.utils.data.DataLoader(dx_train, batch_size, shuffle=True)
    train(resnet18,loss_fn,opt,lr_sched,dl_train,x)
    # torch.save(resnet18.state_dict(),'./data/model.pth')
    Flag = 1
